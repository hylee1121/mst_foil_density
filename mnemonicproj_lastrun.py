...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/hlee/Documents/foilmemoryexp/mnemonicproj_lastrun.py',
...
for thisTotal_loop in total_loop:
    ...
    for thisRcg_block in rcg_block:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if image_stim_rcg.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_stim_rcg.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    image_stim_rcg.tStop = t  # not accounting for scr refresh
                    image_stim_rcg.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(image_stim_rcg, 'tStopRefresh')  # time at next scr refresh
                    image_stim_rcg.setAutoDraw(False)
            ...
            if key_resp_rcg.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp_rcg.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    key_resp_rcg.tStop = t  # not accounting for scr refresh
                    key_resp_rcg.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(key_resp_rcg, 'tStopRefresh')  # time at next scr refresh
                    key_resp_rcg.status = FINISHED
            ...
            if rcg_q.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rcg_q.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    rcg_q.tStop = t  # not accounting for scr refresh
                    rcg_q.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(rcg_q, 'tStopRefresh')  # time at next scr refresh
                    rcg_q.setAutoDraw(False)
            ...
            if rcg_a.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rcg_a.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    rcg_a.tStop = t  # not accounting for scr refresh
                    rcg_a.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(rcg_a, 'tStopRefresh')  # time at next scr refresh
                    rcg_a.setAutoDraw(False)
